...
export class Initial extends LitElement {
  ...
  @property({ type: String }) apiUrl = ''; // Optional API URL, e.g., 'https://initial.sh/api'
  ...
  private async loadPlugins() {
    ...
    // Optional API fetch if apiUrl provided
    if (this.apiUrl) {
      try {
        ...
        this.addOutput('Error: Couldn’t fetch plugins from initial.sh', true);
      }
    }
  }
  ...
  private async emitConversation() {
    ...
    if (this.apiUrl) {
      try {
        ...
        response = 'Error: Couldn’t reach initial.sh!';
      }
    } else {
  ...
  private async submitFeedback() {
    ...
    if (this.apiUrl) {
      try {
        ...
        this.addOutput('Feedback sent to initial.sh! Thanks, matey!');
      } catch (e) {
        this.addOutput('Error: Feedback lost at sea!', true);
      }
    } else {
