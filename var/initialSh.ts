...
export class InitialSh extends LitElement {
  static styles = css` /* Same styles as before */ `;
  ...
  private coreCommands: Record<string, () => void> = {
    help: () => this.addOutput(`Commands: help, clear, exit${Object.keys(this.pluginCommands).length ? ', ' + Object.keys(this.pluginCommands).join(', ') : ''}`),
    clear: () => {
      this.messages = [];
      this.conversation = [];
    },
