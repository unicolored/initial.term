...
@customElement('initial') // Changed from 'initial-sh' to 'initial'
export class Initial extends LitElement {
  ...
  private handleInput(e: KeyboardEvent) {
    ...
    if (e.key === 'Enter' && input) {
      this.addOutput(`> ${input}`, false, false);
  ...
  private async addOutput(text: string, isError = false, typeEffect = true) {
    ...
    if (typeEffect && text.length > 1) {
      ...
      for (const char of text) {
        typedText += char;
        this.messages = [...this.messages.slice(0, index), typedText, ...this.messages.slice(index + 1)];
...
window.Initial = {
  registerPlugin: (name, execute) => {
    const console = document.querySelector('initial') as Initial; // Matches <initial>
