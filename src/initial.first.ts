...
export class Initial extends LitElement {
  ...
  private initSounds() { /* Same as before */ }
  private playSound(frequency: number, type: OscillatorType, duration: number) { /* Same */ }
  ...
  private registerPlugins(plugins: Plugin[]) { /* Same */ }
  private handleInput(e: KeyboardEvent) { /* Same */ }
  private addOutput(text: string, isError = false) { /* Same */ }
  private processCommand(command: string) { /* Same */ }
  private emitConversation() { /* Same */ }
  private fakeResponse(input: string): string { /* Same */ }
  private startFeedback() { /* Same */ }
  private submitFeedback() { /* Same */ }
